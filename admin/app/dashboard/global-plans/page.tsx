...
export default function GlobalPlansPage() {
  ...
  const fetchPlans = async () => {
    try {
      setLoading(true);
      const response = await api.get('/admin/global-plans', {
        params: { page, limit: 10 },
      });
      setPlans(response.data.plans);
      setTotalPages(response.data.pagination.totalPages);
    } catch (error: any) {
      toast.error('Greška pri učitavanju globalnih planova');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Da li ste sigurni da želite da obrišete ovaj globalni plan?')) return;

    try {
      await api.delete(`/admin/global-plans/${id}`);
      toast.success('Globalni plan uspešno obrisan');
      fetchPlans();
    } catch (error: any) {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Globalni Planovi</h1>
          <p className="mt-1 text-sm text-gray-500">Pregled svih globalnih planova</p>
        </div>
