    ...
    <div>
      {paginatedData.map((d, index) => (
        ...
        >
          {d.imageUrl && (
            <div
              className={`${!d.mainNews ? styles.smallImage : ""} ${styles.imageContainer}`}
            >
              <Image
                src={`http://127.0.0.1:8090/api/files/pbc_1315766310/${d.id}/${d.imageUrl}`}
                alt={d.imageDescription}
                fill
            ...
            <div className={styles.personInfo}>
              <div
                className={clsx(styles.personName, {
                  [styles.mainPersonName]: d.mainNews,
                })}
              >
                {d.authorName || d.expand?.user?.name || "Nepoznato"}
              </div>
              <div
                className={clsx(styles.schoolName, {
                  [styles.mainSchoolName]: d.mainNews,
                })}
              >
                {d.currentWork || d.expand?.user?.currentWork
                  ? d.currentWork || d.expand?.user?.currentWork
                  : ""}
              </div>
              <div
                className={clsx(styles.dateAndLikes, {
                  [styles.mainDateAndLikes]: d.mainNews,
                })}
              >
