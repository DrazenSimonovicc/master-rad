...
import { classScheduleService, activityService } from "@/libs/api";
import {
...
const Calendar = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await classScheduleService.delete(deleteId);
  ...
  const classScheduleFormik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (editClassData) {
          await classScheduleService.update(editClassData.id, payload);
        } else {
          await classScheduleService.create(payload);
        }
