...
const LoginPage: FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Nevažeći email ili lozinka");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("auth-change"));
      // Use window.location instead of router.push to force a full page reload
      window.location.href = "/podaci-o-korisniku";
    } catch (err: any) {
      console.error("SignUp error:", err);
      setError(err.response?.data?.error || "Trenutno ne možete kreirati nalog. Pokušajte ponovo kasnije.");
    } finally {
      setLoading(false);
    }
  };
