...
const Homework = () => {
  ...
  const formik = useFormik({
    ...
    initialValues: editingHomework
      ? {
          teachingUnit: editingHomework.teachingUnit,
          tasks: taskKeys
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const dataToSend: Record<string, string | undefined> = {
          teachingUnit: values.teachingUnit,
        ...
        if (editingHomework) {
          await homeworkService.update(editingHomework.id, dataToSend);
        } else {
          await homeworkService.create(dataToSend);
        }
      ...
      <div className={!isLoggedIn ? styles.blurWrapper : ""}>
                  ...
                  {paginatedMyTests.map((t) => {
                    ...
                    return (
                      <div key={t.id} className={styles.teachingUnitWrapper}>
                        <TeachingUnitTitle
                          title={t.teachingUnit}
                          isExpanded={isExpanded}
                          onToggle={() => toggleExpandedUnit(t.id)}
                          onEdit={() => handleEdit(t)}
                          onDelete={() => handleDelete(t.id)}
                          canEdit={t.userId === userData?.id}
                          plan={t}
                          type={"homework"}
                        />
                        {isExpanded && (
        ...
        >
          <form onSubmit={formik.handleSubmit} className={styles.form}>
            <TextField
              label={homeworkConfig.teachingUnit.label}
              placeholder={homeworkConfig.teachingUnit.placeholder}
              variant="outlined"
              fullWidth
              margin="normal"
              name="teachingUnit"
              value={formik.values.teachingUnit}
              onChange={formik.handleChange}
              error={
                formik.touched.teachingUnit &&
                Boolean(formik.errors.teachingUnit)
              }
              helperText={
                formik.touched.teachingUnit && formik.errors.teachingUnit
              }
            />
